...
def consolidate_logs_recursive(directory_path, output_csv, type):
    ...
    if not log_files:
        print(f"在目录 {directory_path} 及其子目录中没有找到_ad_hnsw_{type}_dist_time.log文件")
        return
...
if __name__ == "__main__":
    # 设置路径
    log_directory = r"E:\cppwork\dco_benchmarks\DATA\Finger\finger-avx512"
    output_file = r"E:\cppwork\dco_benchmarks\DATA\Finger\finger-avx512\HNSW_finger_simd.csv"
    
    # 执行整合（递归处理所有子目录）
    print("开始递归整合所有子目录中的0.log文件...")
    result_df = consolidate_logs_recursive(log_directory, output_file, 9)
