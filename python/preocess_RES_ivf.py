...
def extract_dataset_name_hnsw(filename, type):
    ...
    pattern = f'_ad_hnsw_{type}_dist_time$'
    name = re.sub(pattern, '', name)
    return name

def extract_dataset_name_ivf(filename, type):
    """从文件名中提取数据集名称"""
    # 移除文件扩展名
    name = os.path.splitext(filename)[0]
    # 移除HNSW相关的后缀，包括_ad_hnsw_n.log格式
...
def process_log_file_dist_time(filepath, type):
    ...
    with open(filepath, 'r', encoding='utf-8') as f:
        for line in f:
            ...
            if line:  # 跳过空行
                parts = line.split()
                if len(parts) == 5:  # 修改为5列
                    efsearch = int(parts[0])
                    recall = float(parts[1])
                    per_query_us = float(parts[2])
                    # 第4列是per_query_ms，第5列是pruing_rate
                    pruing_rate = float(parts[4])
...
def consolidate_logs_recursive(directory_path, output_csv, type):
    ...
    for root, dirs, files in os.walk(directory_path):
        for file in files:
            # 只处理匹配特定type的log文件
            if file.endswith(f'_ad_hnsw_{type}_dist_time.log'):
                log_files.append(os.path.join(root, file))
    
    if not log_files:
        print(f"在目录 {directory_path} 及其子目录中没有找到_ad_ivf_{type}_dist_time.log文件")
        return
    
    print(f"找到 {len(log_files)} 个_ad_hnsw_{type}_dist_time.log文件")
    ...
    for dataset, files in dataset_files.items():
        ...
        for log_file in files:
            ...
            df = process_log_file_dist_time(log_file, type)
            all_data.append(df)
...
if __name__ == "__main__":
    # 设置路径
    log_directory = r"E:\cppwork\dco_benchmarks\DATA\Resnew\数据规模和维度-d32\recall@10\hnsw\simd"
    output_file = r"E:\cppwork\dco_benchmarks\DATA\Resnew\数据规模和维度-d32\recall@10\hnsw\simd\HNSW_RES_simd_4_dist_time.csv"

    # 执行整合（递归处理所有子目录）
    print("开始递归整合所有子目录中的0.log文件...")
    result_df = consolidate_logs_recursive(log_directory, output_file, 4)
